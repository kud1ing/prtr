...
type CommandFunction = fn(interpreter: &Interpreter, command_name: String, command_arguments: &[String]) -> Evaluation;
...
impl Interpreter {
    ...
    fn evaluate_command(&self, command_parts: &Vec<String>) -> Evaluation {
        ...
        command_function(self, command_name.to_string(), command_arguments)
    }
...
fn command_puts(interpreter: &Interpreter, command_name: String, command_arguments: &[String]) -> Evaluation {
...
fn command_set(interpreter: &Interpreter, command_name: String, command_arguments: &[String]) -> Evaluation {

    if command_arguments.len() != 2 {
        return Err(EvaluationError::ExpectedNumberOfArguments(command_name, 2));
    }

    let name = command_arguments.get(0).unwrap();
    let value = command_arguments.get(1).unwrap();
...
mod tests {
    ...
    fn test_next_token() {
        ...
        let texts_and_expected_tokens = &[
            ...
            ("\n", vec![Some(Token::EndOfCommand)]),
            (";", vec![Some(Token::EndOfCommand)]),
            ("$blub", vec![Some(Token::VariableSubstitution("blub".to_owned()))]),

            ("[command1 arg]", vec![Some(Token::CommandSubstitution("command1 arg".to_owned()))]),
            ("[command2 arg1 arg2]", vec![Some(Token::CommandSubstitution("command2 arg1 arg2".to_owned()))]),
            ("[a [b c]]", vec![Some(Token::CommandSubstitution("a [b c]".to_owned()))]),
